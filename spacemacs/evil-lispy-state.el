;;; evil-lispy-state.el --- An evil state to edit Lisp code using lispy

;; Based off evil-lisp-state (note the lack of a 'y')
;; Copyright (C) 2014, 2015 syl20bnr
;; Copyright (C) 2023 cdo256
;;
;; Authors: Christina O'Donnell <cdo@mutix.org>,
;;          Sylvain Benner <sylvain.benner@gmail.com>
;; Keywords: convenience editing evil lispy lisp mnemonic
;; Created: 9 Oct 2014 by syl20bnr
;; Adapted: 3 June 2023 by cdo256
;; Version: 8.2
;; Package-Requires: ((evil "1.0.9") (bind-map "0") (lispy )) #TODO
;; Original URL: https://github.com/syl20bnr/evil-lisp-state
...
(evil-define-state lispy
 ...
 Used to navigate lisp code and manipulate the sexp tree."
  ...
  (if (evil-lispy-state-p) (lispy-mode 1)))

(defgroup evil-lispy-state nil
  "Evil lispy state."
  :group 'emulations
  :prefix 'evil-lispy-state-)

(eval-and-compile
  (defcustom evil-lispy-state-global nil
    "If non nil evil-lispy-state is available everywhere."
    :type 'boolean
    :group 'evil-lispy-state)

  (defcustom evil-lispy-state-major-modes '(emacs-lisp-mode)
    "Major modes where evil leader key bindings are defined.
If `evil-lispy-state-global' is non nil then this variable has no effect."
    :type 'sexp
    :group 'evil-lispy-state)

  (defcustom evil-lispy-state-enter-lispy-state-on-command t
    "If non nil, enter evil-lispy-state before executing command."
    :type 'sexp
    :group 'evil-lispy-state))

(defvar evil-lispy-state-default-state 'normal
  "The state to activate when exiting lispy state")

(defmacro evil-lispy-state-enter-command (command)
  "Wrap COMMAND to call evil-lispy-state before executing COMMAND."
  (let ((funcname (if (string-match "lispy-state-"
                                    (symbol-name command))
                      (intern (format "evil-%s" command))
                    (intern (format "evil-lispy-state-%s" command)))))
    `(progn
       (defun ,funcname ()
        (interactive)
        (when evil-lispy-state-enter-lispy-state-on-command
          (evil-lispy-state))
        (call-interactively ',command))
       ',funcname)))

(defun evil-lispy-state-escape-command (command)
...
;; leader maps
(defun evil-lispy-state-leader (leader)
  "Set LEADER."
  (bind-map evil-lispy-state-map
    :evil-use-local t
    :evil-keys (leader)
    :evil-states (normal))
  (eval
   `(bind-map evil-lispy-state-major-mode-map
      :evil-keys (,leader)
      :evil-states (normal)
      :major-modes ,evil-lispy-state-major-modes)))

(defun evil-lispy-state/quit ()
  "Quit lisp state and set state `evil-lispy-state-default-state'."
  (interactive)
  (funcall (intern (format "evil-%S-state" evil-lispy-state-default-state))))

(define evil-lispy-state-map 'lispy-mode-map-base)
(define-key evil-lispy-state-map [escape] 'evil-lispy-state-escape-command)
;; toggle lisp state
(define-key evil-lispy-state-map "." 'lispy-state-toggle-lispy-state)
;; hjkl
(define-key evil-lispy-state-map "h" 'evil-backward-char)
(define-key evil-lispy-state-map "j" 'evil-next-visual-line)
(define-key evil-lispy-state-map "k" 'evil-previous-visual-line)
(define-key evil-lispy-state-map "l" 'evil-forward-char)

;; auto-switch to lisp state commands
(defconst evil-lispy-state-commands
  `(("%"   . evil-jump-item)
    (":"   . evil-ex)
    ("("   . lispy-state-insert-sexp-before)
...
(defvar evil-lispy-state-major-mode-map 'lispy-mode-map-base)

(dolist (x evil-lispy-state-commands)
  (let ((key (car x))
        (cmd (cdr x)))
    (eval
     `(progn
        (if evil-lispy-state-global
            (define-key evil-lispy-state-map ,(kbd key)
              (evil-lispy-state-enter-command ,cmd))
          (define-key evil-lispy-state-major-mode-map ,(kbd key)
            (evil-lispy-state-enter-command ,cmd)))))))

(defun lispy-state-toggle-lispy-state ()
  ...
  (if (eq 'lisp evil-state)
    ...
    (evil-lispy-state)))
(defun lispy-state-wrap (&optional arg)
  "Wrap a symbol with parenthesis."
  (interactive "P")
  (sp-wrap-with-pair "("))

(defun evil-lispy-state-next-paren (&optional closing)
...
(defun lispy-state-prev-opening-paren ()
  "Go to the next closing parenthesis."
  (interactive)
  (evil-lispy-state-next-paren))

(defun lispy-state-next-closing-paren ()
  "Go to the next closing parenthesis."
  (interactive)
  (evil-lispy-state-next-paren 'closing))

(defun lispy-state-forward-symbol (&optional arg)
...
(defun lispy-state-insert-sexp-after ()
...
(defun lispy-state-insert-sexp-before ()
...
(defun lispy-state-eval-sexp-end-of-line ()
...
(defun lispy-state-beginning-of-sexp (&optional arg)
...
(provide 'evil-lispy-state)

;;; evil-lispy-state.el ends here
