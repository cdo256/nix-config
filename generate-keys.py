...
if args.regenerate or not ssh_priv_key_path.exists():
...
if args.regenerate or not age_key_path.exists():
    print(f'Generating age key {age_key_path} from ed25519 key...')
    p = sp.run(['nix', 'run', 'nixpkgs#ssh-to-age', '--', '-private-key', '-i', ssh_priv_key_path], capture_output=True)
